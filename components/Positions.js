// components/Positions.js
import React, { useEffect, useState, useMemo } from 'react';
import {
  View,
  Text,
  StyleSheet,
  TouchableOpacity,
  Dimensions,
  Platform,
  ActivityIndicator,
  FlatList,
} from 'react-native';

import Header from './Header';
import AdFooter from '../ads/AdFooter';
import FooterTabs from './FooterTabs';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

import LogoImg from './LogoImg';
import { changeName } from '../utils/changeName';
import TeamSearchModal from './TeamSearchModal';
import analytics from '@react-native-firebase/analytics';
import { useTheme } from '../utils/ThemeContext';

import { API_CUSTOM_DIGITAL } from '@env';

const { width } = Dimensions.get('window');

const COL = {
  RANK: 32,
  PTS: 40,
  NUM: 30,
  DIF: 38,
  ICON: 26,
  GAP: 6,
};
const RIGHT_W = COL.PTS + 4 * COL.NUM + COL.DIF;
const CUM_W = COL.PTS + COL.NUM + COL.DIF;
const TEAM_GUTTER = 20;
const RIGHT_INNER_PAD = -10;
const TAB_H = 58;
const BANNER_PAD = 120;
const padBottom = TAB_H + 6 + BANNER_PAD;

// === Helpers numéricos con fallback ===
const num = (v) => (v == null || v === '' ? 0 : parseInt(v, 10) || 0);

const pick = (row, keys = []) => {
  for (const k of keys) {
    const v = row?.[k]?.[0];
    if (v != null && v !== '') return num(v);
  }
  return 0;
};

const A = (row, base) => pick(row, [`${base}actual`, base]);

const computePTS = (row) => {
  const pv = pick(row, ['puntosactual', 'puntos']);
  if (pv) return pv;
  const w = A(row, 'ganados');
  const d = A(row, 'empatados');
  return w * 3 + d;
};

function SegmentedBtn({ active, label, onPress, colors, mode }) {
  // “gradient fake” sin layers absolutas raras: dos vistas lado a lado
  const bgInactive = colors.cardBg;
  const borderInactive = colors.cardBorder;

  return (
    <TouchableOpacity
      activeOpacity={0.9}
      onPress={onPress}
      style={[
        styles.segBtn,
        {
          borderColor: active ? 'transparent' : borderInactive,
          backgroundColor: active ? 'transparent' : bgInactive,
        },
      ]}
    >
      {active ? (
        <View style={styles.gradWrap} pointerEvents="none">
          <View style={[styles.gradA, { backgroundColor: '#3572fd' }]} />
          <View style={[styles.gradB, { backgroundColor: '#3679fe' }]} />
        </View>
      ) : null}

      <View style={styles.segInner}>
        {/* Punto rojo centrado verticalmente */}
        <View style={[styles.dot, { opacity: active ? 1 : 0 }]} />
        <Text style={[styles.segText, { color: active ? '#fff' : colors.text }]}>
          {label}
        </Text>
      </View>
    </TouchableOpacity>
  );
}

export default function Positions({ navigation }) {
  const [standings, setStandings] = useState([]);
  const [standingsCum, setStandingsCum] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showCum, setShowCum] = useState(true);
  const [showTeamSearch, setShowTeamSearch] = useState(false);
  const [footerVersion, setFooterVersion] = useState(0);

  const insets = useSafeAreaInsets();
  const safeBottom = Math.max(insets?.bottom || 0, 0);

  const { theme } = useTheme();
  const colors = theme.colors;

  useEffect(() => {
    (async () => {
      try {
        await analytics().logScreenView({
          screen_name: 'Positions',
          screen_class: 'Positions',
        });
      } catch {}
    })();
  }, []);

  useEffect(() => {
    fetch(`${API_CUSTOM_DIGITAL}/positions`)
      .then((r) => r.json())
      .then((result) => {
        const teams = result?.posiciones?.equipo || [];
        const cum = [...teams];

        cum.sort((rowA, rowB) => {
          const p1 = computePTS(rowA);
          const p2 = computePTS(rowB);
          if (p1 !== p2) return p2 - p1;
          const dg1 = A(rowA, 'difgol');
          const dg2 = A(rowB, 'difgol');
          return dg2 - dg1;
        });

        setStandings(teams);
        setStandingsCum(cum);
      })
      .catch((err) => console.warn('Error posiciones:', err))
      .finally(() => setLoading(false));
  }, []);

  const data = showCum ? standingsCum : standings;
  const total = data.length;

  const handleCloseSearch = () => {
    setShowTeamSearch(false);
    setFooterVersion((v) => v + 1);
  };

  const HeaderSticky = useMemo(() => {
    return (
      <View style={{ backgroundColor: colors.screenBg }}>
        <View style={styles.tableHeaderWrap}>
          <View
            style={[
              styles.tableHeaderCard,
              {
                backgroundColor: colors.cardBg,
                borderColor: colors.cardBorder,
              },
            ]}
          >
            <View style={[styles.thCol, { width: COL.RANK }]}>
              <Text style={[styles.thText, { color: colors.textMuted }]}>#</Text>
            </View>

            <View style={{ flex: 1, paddingRight: TEAM_GUTTER }}>
              <Text style={[styles.thTextLeft, { color: colors.textMuted }]}>Equipo</Text>
            </View>

            {showCum ? (
              <View style={{ width: CUM_W, flexDirection: 'row', marginRight: RIGHT_INNER_PAD }}>
                <View style={[styles.thCol, { width: COL.PTS }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>Pts</Text>
                </View>
                <View style={[styles.thCol, { width: COL.NUM }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>J</Text>
                </View>
                <View style={[styles.thCol, { width: COL.DIF }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>Dif</Text>
                </View>
              </View>
            ) : (
              <View style={{ width: RIGHT_W, flexDirection: 'row', marginRight: RIGHT_INNER_PAD }}>
                <View style={[styles.thCol, { width: COL.PTS }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>Pts</Text>
                </View>
                <View style={[styles.thCol, { width: COL.NUM }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>J</Text>
                </View>
                <View style={[styles.thCol, { width: COL.NUM }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>G</Text>
                </View>
                <View style={[styles.thCol, { width: COL.NUM }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>E</Text>
                </View>
                <View style={[styles.thCol, { width: COL.NUM }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>P</Text>
                </View>
                <View style={[styles.thCol, { width: COL.DIF }]}>
                  <Text style={[styles.thText, { color: colors.textMuted }]}>Dif</Text>
                </View>
              </View>
            )}
          </View>
        </View>

        <View style={{ height: 8, width, backgroundColor: colors.screenBg }} />
      </View>
    );
  }, [colors, showCum]);

  return (
    <View style={{ flex: 1, backgroundColor: colors.screenBg, position: 'relative' }}>
      <Header
        title="TABLA DE POSICIONES"
        navigation={navigation}
        showSearch
        onSearchPress={() => setShowTeamSearch(true)}
      />

      {loading ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <ActivityIndicator size="large" color={colors.accent} />
        </View>
      ) : (
        <View style={{ flex: 1, minHeight: 0 }}>
          {/* ✅ Selector centrado REAL */}
          <View style={styles.selectorTop}>
            <View
              style={[
                styles.segmented,
                {
                  borderColor: colors.cardBorder,
                  backgroundColor: colors.cardBg,
                },
              ]}
            >
              <SegmentedBtn
                active={showCum}
                label="ACUMULADA"
                colors={colors}
                mode={theme.mode}
                onPress={async () => {
                  setShowCum(true);
                  try {
                    await analytics().logEvent('positions_tab', { table: 'ACUMULADA' });
                  } catch {}
                }}
              />
              <SegmentedBtn
                active={!showCum}
                label="APERTURA"
                colors={colors}
                mode={theme.mode}
                onPress={async () => {
                  setShowCum(false);
                  try {
                    await analytics().logEvent('positions_tab', { table: 'APERTURA' });
                  } catch {}
                }}
              />
            </View>
          </View>

          <FlatList
            style={{ flex: 1 }}
            data={data}
            keyExtractor={(item, index) => String(item?.id?.[0] ?? index)}
            showsVerticalScrollIndicator={false}
            contentContainerStyle={{ paddingBottom: padBottom, paddingTop: 6 }}
            scrollIndicatorInsets={{ bottom: padBottom }}
            keyboardShouldPersistTaps="handled"
            stickyHeaderIndices={[0]}
            ListHeaderComponent={HeaderSticky}
            renderItem={({ item, index }) => {
              const teamId = item?.id?.[0] ? String(item.id[0]) : null;
              const name = changeName((item?.nombre?.[0] || '').trim());

              const PTS = computePTS(item);
              const J = A(item, 'jugados');
              const G = A(item, 'ganados');
              const E = A(item, 'empatados');
              const P = A(item, 'perdidos');
              const DIF = A(item, 'difgol');

              const rank = index + 1;
              const qual = rank <= 8;
              const bottom = rank >= total - 1;

              const rowDangerBg =
                theme.mode === 'dark' ? 'rgba(220,38,38,0.12)' : '#FEF2F2';

              const cardBg = bottom ? rowDangerBg : colors.cardBg;

              const circleStyle = bottom
                ? [styles.rankCircle, styles.rankCircleDanger]
                : qual
                ? [styles.rankCircle, styles.rankCircleQual]
                : [
                    styles.rankCircle,
                    {
                      backgroundColor:
                        theme.mode === 'dark' ? 'rgba(255,255,255,0.04)' : '#F3F4F6',
                      borderColor: colors.cardBorder,
                    },
                  ];

              const circleTextStyle = bottom
                ? [styles.rankText, styles.rankTextDanger]
                : qual
                ? [styles.rankText, styles.rankTextQual]
                : [styles.rankText, { color: colors.text }];

              return (
                <View style={styles.rowOuter}>
                  <View
                    style={[
                      styles.rowCard,
                      {
                        backgroundColor: cardBg,
                        borderColor: colors.cardBorder,
                      },
                    ]}
                  >
                    <View style={{ width: COL.RANK }}>
                      <View style={circleStyle}>
                        <Text style={circleTextStyle}>{rank}</Text>
                      </View>
                    </View>

                    <TouchableOpacity
                      activeOpacity={0.7}
                      onPress={async () => {
                        const idNum = Number(teamId);
                        if (!Number.isFinite(idNum)) return;
                        try {
                          await analytics().logEvent('open_team_from_positions', {
                            team_id: String(teamId || ''),
                            team_name: String(name || ''),
                            rank: Number(rank),
                            pts: Number(PTS),
                            table: showCum ? 'ACUMULADA' : 'APERTURA',
                          });
                        } catch {}
                        navigation?.push?.('TeamScreen', {
                          teamId: idNum,
                          scope: 'guatemala',
                          tab: 'Resumen',
                          teamName: name,
                        });
                      }}
                      style={{
                        flex: 1,
                        flexDirection: 'row',
                        alignItems: 'center',
                        paddingRight: TEAM_GUTTER,
                      }}
                    >
                      <LogoImg teamId={teamId} size={COL.ICON} style={{ marginRight: COL.GAP }} />
                      <Text style={[styles.tdTeamName, { color: colors.text }]} numberOfLines={1}>
                        {name}
                      </Text>
                    </TouchableOpacity>

                    {showCum ? (
                      <View style={{ width: CUM_W, flexDirection: 'row', marginRight: RIGHT_INNER_PAD }}>
                        <Text style={[styles.colNum, styles.colPts, { width: COL.PTS, color: colors.text }]}>
                          {PTS}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.NUM, color: colors.textMuted }]}>
                          {J}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.DIF, color: colors.textMuted }]}>
                          {DIF}
                        </Text>
                      </View>
                    ) : (
                      <View style={{ width: RIGHT_W, flexDirection: 'row', marginRight: RIGHT_INNER_PAD }}>
                        <Text style={[styles.colNum, styles.colPts, { width: COL.PTS, color: colors.text }]}>
                          {PTS}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.NUM, color: colors.textMuted }]}>
                          {J}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.NUM, color: colors.textMuted }]}>
                          {G}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.NUM, color: colors.textMuted }]}>
                          {E}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.NUM, color: colors.textMuted }]}>
                          {P}
                        </Text>
                        <Text style={[styles.colNum, { width: COL.DIF, color: colors.textMuted }]}>
                          {DIF}
                        </Text>
                      </View>
                    )}
                  </View>
                </View>
              );
            }}
          />
        </View>
      )}

      <TeamSearchModal
        visible={showTeamSearch}
        onClose={handleCloseSearch}
        onSelect={async (team) => {
          try {
            await analytics().logEvent('open_team_from_search', {
              team_id: String(team?.id ?? ''),
              team_name: String(team?.name ?? ''),
              scope: String(team?.scope ?? 'guatemala'),
            });
          } catch {}
          navigation.navigate('TeamScreen', {
            teamId: team.id,
            scope: team.scope,
            teamName: team.name,
          });
          handleCloseSearch();
        }}
      />

      <View key={footerVersion} style={[styles.footerFixed, { height: TAB_H + safeBottom }]}>
        <View style={{ position: 'absolute', left: 0, right: 0, bottom: TAB_H + safeBottom, zIndex: 20, elevation: 20 }}>
          <AdFooter />
        </View>

        <View style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: TAB_H + safeBottom, zIndex: 10, elevation: 10 }}>
          <FooterTabs navigation={navigation} routeName="Positions" />
        </View>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  selectorTop: {
    paddingHorizontal: 12,
    paddingTop: 8,
    paddingBottom: 6,
    alignItems: 'center',
  },

  // pill centrado
  segmented: {
    width: 300,              // ✅ centrado real
    maxWidth: '95%',
    flexDirection: 'row',
    borderRadius: 999,
    borderWidth: 1,
    padding: 3,
    overflow: 'hidden',

    ...Platform.select({
      ios: {
        shadowColor: '#000',
        shadowOpacity: 0.06,
        shadowRadius: 10,
        shadowOffset: { width: 0, height: 4 },
      },
      android: { elevation: 2 },
      default: {},
    }),
  },

  segBtn: {
    flex: 1,
    height: 36,
    borderRadius: 999,
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'hidden',
  },

  // fake gradient container
  gradWrap: {
    ...StyleSheet.absoluteFillObject,
    flexDirection: 'row',
  },
  gradA: { flex: 1 },
  gradB: { flex: 1 },

  segInner: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 8,
    paddingHorizontal: 8,
  },

  dot: {
    width: 8,
    height: 8,
    borderRadius: 99,
    backgroundColor: '#fc2729',
  },

  segText: {
    fontSize: 12.5,
    fontWeight: '900',
    letterSpacing: 0.3,
    includeFontPadding: false,
    textAlignVertical: 'center',
    fontFamily: Platform.OS === 'android' ? 'Roboto' : 'System',
  },

  tableHeaderWrap: {
    paddingHorizontal: 12,
    paddingTop: 6,
    paddingBottom: 6,
  },
  tableHeaderCard: {
    flexDirection: 'row',
    paddingHorizontal: 10,
    paddingVertical: 10,
    borderWidth: 1,
    borderRadius: 14,
  },

  thCol: { alignItems: 'center', justifyContent: 'center' },
  thText: {
    fontSize: 12,
    fontWeight: '900',
    includeFontPadding: false,
    textAlignVertical: 'center',
    fontFamily: Platform.OS === 'android' ? 'Roboto' : 'System',
  },
  thTextLeft: {
    fontSize: 12,
    fontWeight: '900',
    includeFontPadding: false,
    textAlignVertical: 'center',
    fontFamily: Platform.OS === 'android' ? 'Roboto' : 'System',
  },

  rowOuter: { paddingHorizontal: 12, paddingVertical: 6 },
  rowCard: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 10,
    paddingVertical: 12,
    borderWidth: 1,
    borderRadius: 14,
    ...Platform.select({
      ios: {
        shadowColor: '#000',
        shadowOpacity: 0.05,
        shadowRadius: 10,
        shadowOffset: { width: 0, height: 4 },
      },
      android: { elevation: 1 },
      default: {},
    }),
  },

  tdTeamName: {
    fontSize: 13.5,
    fontWeight: '800',
    includeFontPadding: false,
    textAlignVertical: 'center',
    fontFamily: Platform.OS === 'android' ? 'Roboto' : 'System',
  },

  rankCircle: {
    width: 24,
    height: 24,
    borderRadius: 12,
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 1,
  },
  rankCircleQual: { backgroundColor: '#10B981', borderColor: '#059669' },
  rankCircleDanger: { backgroundColor: '#EF4444', borderColor: '#DC2626' },

  rankText: { fontSize: 13, fontWeight: '900' },
  rankTextQual: { color: '#fff' },
  rankTextDanger: { color: '#fff' },

  colNum: {
    fontSize: 13,
    textAlign: 'center',
    fontWeight: '700',
    minWidth: 24,
    includeFontPadding: false,
    textAlignVertical: 'center',
    fontFamily: Platform.OS === 'android' ? 'Roboto' : 'System',
    ...(Platform.OS === 'ios' ? { fontVariant: ['tabular-nums'] } : null),
  },
  colPts: { fontWeight: '900' },

  footerFixed: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    pointerEvents: 'box-none',
  },
});
